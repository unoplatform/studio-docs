...
The agent has a fixed context window. When you reach it, Uno Platform Studio tries to compact the conversation automatically before asking you to take action.
...
Uno Platform Studio uses a credit-based model. See [Credits & Usage](xref:Uno.PlatformStudio.Interface#credits--usage) for the full breakdown.
...
| **An unexpected error occurred. Please retry or restart the application.** | A generic error reached the login page. | Use **Retry**. If the issue persists, restart Uno Platform Studio. |

## Starting a New Project

When you start a new project while a session is active, Uno Platform Studio confirms before discarding state.
...
If you see a **Critical failure** card, or repeatedly hit an error you cannot work around, use **More > Feedback** to send a feedback bundle. The bundle includes the conversation, logs, and project state needed for the Uno Platform team to investigate.
